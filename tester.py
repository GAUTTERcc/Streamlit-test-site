...
if os.path.exists(file_path):
    ...
    if 'date' in df.columns:
        ...
        if 'retweets' in df.columns:
            activity['avg_retweets'] = df.groupby('month')['retweets'].mean().values

        st.subheader("📈 Активність користувача по місяцях")
        fig = px.bar(activity, x='month', y='tweets_count', text='tweets_count',
                     title="Кількість твітів по місяцях")

        if 'avg_likes' in activity.columns:
            fig.add_scatter(x=activity['month'], y=activity['avg_likes'],
                            mode='lines+markers', name='Середні лайки')
        if 'avg_retweets' in activity.columns:
            fig.add_scatter(x=activity['month'], y=activity['avg_retweets'],
                            mode='lines+markers', name='Середні репости')
